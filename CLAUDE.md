...
**Living-map graph (SYN-66, `graph_layout.py` + `graph_clusters.py`)**: `GET /graph` assembles a projection (no new source of truth), from entities ∪ atomic_notes as nodes, relations + mentions as edges, then Louvain clustering (`graph_communities.py`, resolution 1.0 — hand-written and **shared line for line with the Rust core** (`snapshot.rs::assign_communities`) so the HTTP map and the offline map show the same zones; networkx's Louvain was dropped because it shuffles node order with a seed, which no other language can reproduce, and its seed=42 draw was a poor partition), ForceAtlas2 layout persisted/incremental in `node_positions`, and batched+cached Haiku cluster labels (`cluster_labels`, keyed by a signature of the cluster's defining entities) + pure-Python convex hulls. A community must hold ≥`MIN_CLUSTER_SIZE` (3) nodes to become a region: smaller ones aren't forced into a zone; the frontend (SYN-64) floats them as orphans. `semantic_edges` (SYN-64) adds embedding-kNN soft springs (top-4 cosine ≥ 0.62, weight `0.45×score`, **never returned to the client and never counted in a node's degree**) so vector-similar entities drift together and, since they also feed the clustering, group into the same zone. The core computes the same ones (`snapshot.rs::semantic_edges`), pinned to a shared fixture on both sides. The floor was 0.80, which was above the data (median best neighbour 0.626 → 2 edges in production); measured on the real memory, 0.62 takes orphans from 40% to 29% and yields 6 zones instead of 5, while 0.55 collapses modularity to 0.669. `semantic_layout=false` disables. **Dep `networkx>=3.2`** (pure-Python; packages into the PyInstaller .dmg, unlike igraph/leidenalg) — now only for `forceatlas2_layout`, no longer for clustering. Visual mapping: size = `memory_strength`×`degree`, colour = `community_id`, saturation = `memory_strength`, position = `node_positions`. See `docs/ARCHITECTURE.md` §5.
