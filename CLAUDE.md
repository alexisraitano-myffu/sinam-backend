...
- **Map layout: client-side, then back into the core (SYN-64, then 2026-08-25)**: the mobile app briefly computed the living-map layout itself (a vis-network `forceAtlas2Based` port, `ForceLayout.kt`). That file is **gone**: the same solver now lives in the core, and positions arrive with the graph whether the answer comes from the network or from the local snapshot. The app reads the map instead of redrawing it — which is also how semantic springs finally count in the mobile layout, since they are never returned as edges and the app could not know them. The « Réorganiser » button went with it: positions are not persisted and the solver is deterministic, so a second request redrew exactly the same map.
...
- **Labelled corpus + review tool (SYN-184)**: `scripts/parity/corpus/*.jsonl`, one line per
...
FastAPI app for the mobile/desktop clients (run `python -m api`, port 8000), **87 endpoints**; the frozen contract is `openapi.json` (regenerate via `app.openapi()` when it changes: the app codes against it). Bearer auth via `SYNAPSE_API_TOKEN` (auth **disabled** if unset: dev). Core: `GET /health`, `POST /capture` (**idempotent on client UUID**), `GET /feed`, `GET /graph` (living-map SYN-66: base = entities+relations; opt-in flags `include_notes` adds atomic_notes as `n:<id>` nodes + mention edges, `cluster` → `community_id` (Louvain), `layout` → `x`/`y` (ForceAtlas2Based from the core, never persisted; `relayout` accepted and ignored), `clusters` → `{label, hull}` regions; filters `node_types`/`memory_strength_min`/`since`/`top_pct_per_cluster`/`include_isolated`/`max_nodes`), `GET /entity/{id}` (`?include=archived,obsolete`), `GET /atomic-note/{id}` (single note + `provenance_content`, SYN-64), `GET /pending`, `POST /pending/{id}/validate`, `POST /dream-cycle/run` (file lock + `cycle_runs`), `GET /dream-cycle/last`, `GET /changes`, `GET /atomic-notes`, `GET /projects`, `GET /project/{id}/state`, project-entry ops. **Entity-graph endpoints**: `GET /entity/{id}/similar` (SYN-62), `GET/POST /entity-type-proposals*` (SYN-58), `GET/POST /merge-proposals*` (SYN-39), `POST /entity|fact/{id}/archive|unarchive` + `/fact/{id}/obsolete|restore` (SYN-59). **Three more proposal queues, same shape, same invariant (propose, never apply)**: `GET/POST /predicate-proposals*` (SYN-190, a first-seen predicate that looks like one already in use), `GET/POST /negation-proposals*` (SYN-189, a capture negating a fact whose target is not certain — `accept` REQUIRES `fact_id`, since choosing which reading was right is exactly what the core refused to guess), `GET/POST /rename-proposals*` (SYN-188, a rename declared in a capture; `accept` writes through the same path as `PATCH /entity`, so there is one rename path and not two). **Une cinquième file, la création d'entité** : `GET/POST /entity-creation-proposals*`. Une fiche naissait dès qu'un nom apparaissait dans une capture laissant une note durable — la seule des quatre conditions de `should_create` qui ne repose sur aucune preuve. Elle propose désormais. Deux différences avec ses sœurs, toutes deux dues au fait que l'objet n'existe pas encore : aucune clé étrangère vers `entities`, et la charge complète du classifieur est stockée pour qu'accepter n'exige pas de rejouer la capture. L'acceptation est **déléguée au core** (`Brain::accept_entity_creation`), `upsert_entity` restant le seul chemin de création. Repli : `SYNAPSE_ENTITY_CREATE_UNPROVEN=1`. **None of the four has a screen in the app: they answer, but stay write-only until a client renders them.** **Digest/note endpoints (SYN-23)**: `POST /digest/run`, `GET /digest/latest`, `POST /atomic-note/{id}/reinforce`, `POST /atomic-note/{id}/date`. **SYN-91**: `/changes` carries `embedding_b64` per entity (offline « entités liées »). Per-request connections through the core's SQL gateway (`db/__init__.py`, apsw surface, no apsw). Sync model: captures carry `id`/`device_id`/`captured_at`, validations are append-only events → state rebuildable (see `docs/ARCHITECTURE.md`).
