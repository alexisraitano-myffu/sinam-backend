#!/usr/bin/env bash
# Build a standalone macOS bundle of the sinam backend via PyInstaller.
# Output: dist/sinam-backend/  (onedir, ~80-150 MB depending on deps)
#
...
if ! python -c "import sinam_core" 2>/dev/null; then
    echo "[error] sinam_core absent de la venv. Builder la wheel :" >&2
    echo "        cd ../sinam-core && maturin build --release -m crates/sinam-core-py/Cargo.toml" >&2
    echo "        pip install ../sinam-core/target/wheels/sinam_core-*.whl" >&2
    exit 1
fi
...
if [ ! -f "$PROMPTS_SRC/manifest.json" ]; then
    echo "[error] $PROMPTS_SRC introuvable — le bundle a besoin des prompts du repo sinam-core." >&2
    exit 1
fi
mkdir -p dist/sinam-backend/prompts
cp "$PROMPTS_SRC"/*.md "$PROMPTS_SRC"/manifest.json dist/sinam-backend/prompts/
echo "[prompts] $(ls dist/sinam-backend/prompts | wc -l | tr -d ' ') fichiers embarqués"
...
echo
echo "[done] Bundle at: $(pwd)/dist/sinam-backend/"
echo "       Test it with: ./dist/sinam-backend/sinam-backend"
