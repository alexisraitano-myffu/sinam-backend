...
| G-HABIT | habitude ou trait biographique sans moment situé | `x-habitual-past` | **Les deux côtés depuis le 28/08.** « Je suis matinal, je me lève toujours avant 6h » ne laisse rien, « Hier j'ai remarqué que je suis matinal » doit sortir en épisode. Le second échoue, le moteur rend une note de type `note` et sans date : même famille que la ligne 3 qui ne sort pas. |
| G-SVO | garde-fou SVO : si tout se reformule en triplets, c'est un fait | `f1`, `f2`, `r1`, `r2` (indirect) | **Couvert depuis le 28/08, et le garde-fou mange bien la note.** Trois cas : la documentation pure (« Nadia est directrice chez Globex depuis mars ») ne laisse rien, ce qui est juste ; mais l'opinion (« Marc devrait vraiment changer de poste ») et le changement d'avis (« Alors finalement Sophie ne vient pas travailler chez nous ») ne laissent rien non plus, alors qu'ils portent un mouvement. Le prédit du verdict précédent est mesuré. |
...
| R3e | un épisode A une date, et une date passée qui REVIENT prend `event_recurring` | `x-past-recurring-date`, plus la paire yoga / piano du 28/08 | **Toujours ouvert.** La paire est écrite mais les deux côtés attendent l'arbitrage sur l'action passée ordinaire : le moteur ne rend aucune note sur ni l'un ni l'autre, donc `recurring` n'est même pas atteint. Deux captures de la vague ont été écartées, une au présent (une routine n'est pas un épisode) et une portant une négation qui brouillait la variable. |
